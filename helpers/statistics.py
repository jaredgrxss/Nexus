...
def half_life(data: list[float]) -> float:
    ...
    The half-life is the time it takes for the time series to revert halfway back to its mean
    after a deviation. It is calculated using the slope of the regression of the time series
    ...
    Returns:
        float: The half-life of the mean-reverting time series. If the time series is not
            mean-reverting (slope >= 0), returns `None`.
    """
...
def cointegration_adf_test(X: list[float], Y: list[float], lag: int = 1) -> dict:
    """
    Perform the Cointegration Augmented Dickey-Fuller (CADF) Test on two time series.

    This test checks whether two time series are cointegrated by:
    ...
    Args:
        ...
        lag (int, optional): The number of lags to include in the ADF test. Defaults to 1.
    ...
    Returns:
        dict: A dictionary containing the ADF test results, including:
              ...
              - 'critical_values': Critical values for the ADF test at 1%, 5%, and 10%.
              - 'is_cointegrated': A boolean indicating whether the series are cointegrated
...
def johansen_test(data: list[list[float]], det_order: int = - 1, k_ar_diff: int = 1) -> dict:
    """
    Perform the Johansen Test for cointegration on multiple time series.

    The Johansen Test determines the number of cointegrating relationships among a set of
    time series. It is based on the maximum likelihood estimation of a Vector Error
    Correction Model (VECM).

    Args:
        data (list[list[float]]): A list of time series, where each time series is a list of floats.
        det_order (int, optional): The order of the deterministic trend to include in the test.
                                   ...
                                   Defaults to -1.
        k_ar_diff (int, optional): The number of lags in the VAR model. Defaults to 1.
    ...
    Returns:
        dict: A dictionary containing the Johansen Test results, including:
              ...
              - 'critical_values': The critical values for the trace statistics at 90%, 95%, and 99%.
              - 'cointegration_rank': The estimated number of cointegrating relationships.
    """
    
    # Convert the input data to a numpy array
    data = np.array(data).T # Transpose the data to match the expected shape (n_samples, n_series)
...
def bollinger_bands(data: list[float], window: int, num_std: float = 2) -> dict:
    """
    Calculate the Bollinger Bands for a time series.

    Bollinger Bands consist of:
    - Middle Band: Simple Moving Average (SMA) of the time series.
    - Upper Band: SMA + (num_std * standard deviation of the time series).
    - Lower Band: SMA - (num_std * standard deviation of the time series).

    Args:
        data (list[float]): A list of float values representing the time series.
        window (int): The window size for the moving average and standard deviation.
        num_std (float, optional): The number of standard deviations to use for the bands.
                                   Defaults to 2.
    ...
    rolling_std = np.sqrt(np.convolve((data - np.convolve(data, np.ones(window), 'valid') / window)**2, np.ones(window), 'valid') / window)
    ...
    middle_band = np.pad(middle_band, (pad_length, 0), mode='constant', constant_values=np.nan)
    upper_band = np.pad(upper_band, (pad_length, 0), mode='constant', constant_values=np.nan)
    lower_band = np.pad(lower_band, (pad_length, 0), mode='constant', constant_values=np.nan)
...
def linear_regression(X: list[float], Y: list[float]) -> tuple[float, float]:
    """
    Perform simple linear regression to fit a line to the data using scikit-learn.
    ...
    Returns:
        Tuple[float, float]: A tuple containing the slope (a) and intercept (b) of the fitted line.
    """
    if len(X) != len(Y):
        raise ValueError("X and Y must have the same length.")
...
def multiple_regression(X: list[list[float]], Y: list[float]) -> list[float]:
    """
    Perform multiple linear regression to fit a hyperplane to the data using scikit-learn.

    The model is of the form: Y = b0 + b1*X1 + b2*X2 + ... + bn*Xn, where:
    - b0 is the intercept.
    - b1, b2, ..., bn are the coefficients for the independent variables.

    Args:
        X (list[list[float]]): A list of lists, where each inner list represents an independent variable.
        Y (list[float]): The dependent variable (response).

    Returns:
        list[float]: A list containing the intercept (b0) and coefficients (b1, b2, ..., bn).
    """
    if len(X) != len(Y):
        raise ValueError("X and Y must have the same length.")
